...
const DEFAULT_VIEWPORT_COLS := 23
const DEFAULT_VIEWPORT_ROWS := 15
const STYLE_DIRS := {
	0: "overworld",
	1: "underground",
	2: "underwater",
...
func _render_area(map_style: int, area_index: int, bg_name: String) -> void:
	...
	# Read viewport from area JSON (defaults to current 23x15 to preserve behavior).
	var area: Dictionary = current_areas[area_index]
	var vp: Dictionary = area.get("viewport", {})
	var vp_cols: int = int(vp.get("cols", DEFAULT_VIEWPORT_COLS))
	var vp_rows: int = int(vp.get("rows", DEFAULT_VIEWPORT_ROWS))

	_apply_camera(grid_size, vp_cols, vp_rows)
...
func _apply_camera(grid_size: Vector2i, vp_cols: int, vp_rows: int) -> void:
	var cam := player.get_node_or_null("Camera2D") as Camera2D
	if cam == null:
		return

	# Limits = map bounds. Camera2D will clamp itself against these and stop
	# scrolling when the viewport edge reaches the map edge. If map size in
	# some axis is <= viewport size in that axis, the camera can't scroll
	# there at all (limit range degenerate / smaller than viewport).
	cam.limit_left = 0
	cam.limit_top = 0
	cam.limit_right = grid_size.x * TILE_SIZE
	cam.limit_bottom = grid_size.y * TILE_SIZE

	# Zoom = window pixels / viewport pixels (uniform). Pick the axis that
	# requires more zoom-out so both viewport dims fit inside the window;
	# the other axis gets letterboxed.
	var view_w: float = float(vp_cols) * TILE_SIZE
	var view_h: float = float(vp_rows) * TILE_SIZE
	var zoom: float = minf(VIEWPORT_W / view_w, VIEWPORT_H / view_h)
